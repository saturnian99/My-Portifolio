...
const getBetterDiscipline = (better, discipline) => {
  if (discipline.nota < better.nota) return better;
  return discipline;
};

const listBetterDiscipline = students => {
  return students.map(student => (
    { name: student.nome, materia: student.materias.reduce(getBetterDiscipline).name }
  ));
};
...
assert.deepEqual(listBetterDiscipline(estudantes), expected);
